...
function Company({ companyTicket }: Props) {
  ...
  const warningTooltip = (
    <>
      <strong>
        <WarningIcon className="text-warning align-top" width={18} height={18} />
        No support
      </strong>
      <div>This client has no support. Please collect payment before proceeding.</div>
    </>
  );
  ...
  const errorTooltip = (
    <>
      <strong>
        <StopIcon className="text-danger align-top" width={18} height={18} />
        COD
      </strong>
      <div>{`${companyTicket.codreason ? companyTicket.codreason : 'This client is COD. Please collect payment before proceeding.'}`}</div>
    </>
  );
  ...
  const infoTooltip = (
    <>
      ...
      {titleCase(companyTicket.addr1)}
      <br />
      {titleCase(companyTicket.addr2)}
      <br />
      {titleCase(companyTicket.city)}
      <br />
  ...
  return (
    <>
      <h3 className="title">
        {warning && (
          <Popover content={warningTooltip}>
            <WarningIcon className="text-warning align-top" width={28} height={28} />
          </Popover>
        )}
        {error && (
          <Popover content={errorTooltip}>
            <StopIcon className="text-danger align-top" width={28} height={28} />
          </Popover>
        )}
        <Popover content={infoTooltip}>{titleCase(companyTicket?.company)}</Popover>
      </h3>
    </>
  );
}
