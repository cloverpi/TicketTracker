...
function TicketAccourdian({ company }: Props) {
  ...
  return (
    <>
      <div className="border rounded-4 shadow-sm overflow-auto mb-4" ref={containerRef} style={{ height: '201px' }} tabIndex={-1}>
        <div className="accordion accordion-flush mb-2" id="ticketAccordian">
          {lastTickets.map((t, i) => (
            <div key={t.serviceid} className="accordion-item" ref={(el) => (itemRefs.current[i] = el)}>
              ...
              >
                <div className="accordion-body">{t.solution}</div>
              </div>
            </div>
          ))}
        </div>
