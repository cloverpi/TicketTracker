...
function CompanyTeamviewer({ devices }: Prop) {
  ...
  return (
    <>
      <div className="border rounded-4 shadow-sm overflow-auto mb-4" style={{ height: '201px' }}>
        <table
            ...
            {devices.map((t, i: number) => (
              ...
              >
                <td className="status-cell">
                  <span className={`status ${t.online_state == 'Online' ? 'bg-success' : 'bg-danger'}`} />
                </td>
                <td className="text-truncate" onClick={() => tvStationClick(i)}>
                  {t.alias}
                </td>
                <td>
                  {editingRow === i ? (
