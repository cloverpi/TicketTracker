...
const secretBuf = Buffer.from(
  '',
  'utf-8'
);
...
export async function getSettings() {
  const settings: RegistrySettings = await new Promise((resolve, reject) => {
    programKey.values((err, items) => {
      ...
      items.forEach((i) => {
        realkey[i.name as keyof RegistrySettings] = i.value;
      });
      resolve(realkey);
    });
  });
